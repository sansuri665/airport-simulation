...
GLOBAL_DOWNSTREAM_CSV_FILES = ("global_macro_feedback_seed_sweep.csv",)
...
def sync_variant_downstream_csv(
    variant_dir: Path,
    output_root: Path,
    *,
    copy_file: Callable[[Path, Path], Any],
    ...
    """Refresh CSV inputs consumed by standalone downstream model commands.

    Versioned Viewer releases are the only browser data source.  This export is
    deliberately limited to tabular model inputs and must never copy Viewer JS,
    JSON indices, or chunk directories into the canonical output tree.
    """

    copied: list[str] = []
    global_source = variant_dir / "global_macro"
    global_target = output_root / "global_macro"
    for name in GLOBAL_DOWNSTREAM_CSV_FILES:
    ...
    reconciled_target = output_root / "regional_macro_reconciled"
    copied.extend(
        copy_files(
            reconciled_source,
            reconciled_target,
    ...
    regional_target = output_root / "regional_macro"
    ...
    aviation_target = output_root / "regional_aviation_demand"
    ...
    supply_target = output_root / "regional_air_capacity_supply"
    ...
    city_airport_target = output_root / "city_airport_market_demand"
    ...
    quarterly_operations_target = (
        output_root / "city_airport_quarterly_operations"
    )
    ...
    financial_state_target = output_root / "city_airport_financial_state"
...
def build_global_viewer_bundle(
    ...
    *,
    ...
    required = (global_data, reconciliation_data, lazy_index)
    missing = [str(path) for path in required if not path.is_file()]
    if missing:
        raise FileNotFoundError(
            "missing Release-only global Viewer assets: " + ", ".join(missing)
        )
    return "".join(
        [
            f"/* Atomic airport Viewer release: {release_id} */\n",
            viewer_script_source(global_data, ""),
            viewer_script_source(reconciliation_data, ""),
            viewer_script_source(lazy_index, ""),
