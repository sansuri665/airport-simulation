...
- 完整模型 Run；
- 全球宏观、城市航空市场和有效客流预测 Viewer；
- 可按 Seed 运行、查看城市市场并按季度测试北京机场经营的动态页面；
...
当前技术基线为 Python 3.13、`airport-model-v0.15`、`airport-model-output-v5` 和 `seed-explorer-api-v4`。区域百万人次继续采用 reference-only 解释；城市潜在需求、航司计划投放、可承接供给和机场最终吞吐分别保留独立口径。
...
- 客流总量、五类结构、航司长期均衡、机场容量和季度经营消费者已经完成同链路守恒与真实 Seed 验收。
