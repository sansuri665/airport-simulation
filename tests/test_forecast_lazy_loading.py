...
class ForecastLazyLoadingTests(unittest.TestCase):
    ...
    def test_forecast_viewer_requires_lazy_loader_without_full_data_fallback(self) -> None:
        ...
        data_client_js = (STATIC_DIR / "js" / "beijing-forecast" / "data-client.js").read_text(encoding="utf-8")
        ...
        self.assertIn("AirportForecastBootstrap", bootstrap_js)
        self.assertIn("beijing_potential_passenger_forecast_viewer", bootstrap_js)
        ...
        self.assertIn("window.AIRPORT_FORECAST_LAZY_INDEX", bootstrap_js)
        self.assertNotIn("legacyPayload", page_js)
        self.assertNotIn("window.CITY_AIRPORT_POTENTIAL_PASSENGER_FORECAST_DATA", page_js)
        self.assertIn('client.getIndex(state.seedContext, "audit")', page_js)
        self.assertIn("/api/forecast-viewer/index", data_client_js)
        self.assertIn("/api/forecast-viewer/report", data_client_js)
        self.assertIn("activeIndex.contextKey", page_js)
