...
当前中国大陆已有 47 个城市市场配置，但只有北京具备完整的五层配置。上海和广州已经分别存在 `shanghai_airport_system.json` 和 `guangzhou_airport_system.json`，扩展时应补齐下游配置，不要再创建第二份同 ID 的城市市场文件。
...
- `timeline`，并与其它四层保持一致；
- 该城市能获得哪些报告来源、最大预测期、质量、校准、偏差上限和误差带；
- 如果当地数据质量或行业透明度不同，不能默认沿用北京 13 份报告的能力梯度。

`forecast_report_source` 是预测行为的来源标签，不是一个会自动读取外部机构数据的文件路径。模型以隐藏的城市市场真实路径为校验对象，按报告质量、滞后、偏差和误差带生成 as-of 报告。普通报告的 `future_peek_mode` 应为 `false`；God 报告是明确的调试/上帝视角，不属于正常平衡。

### 可以复用

如果所有城市共享同一套“玩家可购买研究产品”，报告 ID、等级结构和通用算法可以先复制；但数值仍需用该城市多 Seed 误差分布重新校准。预测报告不会反向改变真实客流。
...
2. 上海/广州优先审查并修订已有城市市场配置；真正的新城市才新增市场文件。
3. 单独跑城市市场，确认潜在客流、航司供给、五类结构和容量路径合理。
