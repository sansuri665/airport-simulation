...
北京基准经营配置中的静态 `facility_renovation_events`、`facility_construction_events` 和 `facility_rebuild_events` 只用于 reference/replay。正式玩家存档只保存玩家行动日志；`simulate_default` 每次重算时会清空静态示例，并完全按存档中的玩家工程行动重建三类事件。因此大兴 T2 的 2065–2069 示例工程不会自动进入新建存档。
