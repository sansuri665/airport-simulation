...
| `feedback_iterations` | 3 | 宏观反馈回跑次数 |
...
模型先生成一条完整路径，再根据信用、美元、资产、油价、政策和通胀结果推导下一轮的滞后反馈，按默认设置回跑 3 次。反馈有平滑、松弛和上下限，目的是避免单个冲击在循环中无限放大；它不是求解一个严格的经济学均衡。
...
- 反馈回跑只检查预设指标的变化，不保证所有变量达到数学意义上的均衡。
