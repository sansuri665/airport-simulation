...
区域模型把全球变量按本区敏感度转换为本地增长、通胀、利率、汇率、信用、资产、收入、消费信心、能源和压力指数。每个变量带有惯性和边界，区域不会机械复制全球曲线。
...
5. 对通胀、政策利率、10 年期收益率、信用利差、压力、股票回报/估值和能源成本做有上限的共同偏移，使加权结果向全球锚靠近。

第 5 步是“软对账”，不保证每个加权指标与全球值完全相等。这样可以保留区域差异，同时避免用对账层掩盖不合理的区域配置。诊断字段会记录调整前后缺口以及 `low_gap`、`medium_gap` 或 `high_gap` 质量标签。
...
读取下游模型时，应优先使用带 `_reconciled` 的增长、通胀、利率、信用、压力、资产和能源字段；原始字段保留用于解释区域自身生成逻辑。

## 与其他模块的关系

- 上游：全球宏观提供共同世界、风险观察和已激活岔路。
- 同层：对账将 14 条区域路径约束回全球总量。
- 下游：区域航空需求读取对账字段，同时读取区域原始路径中更细的收入、信心、流动性、风险偏好和 seed 势能。
...
- 区域逐年 CSV 目前由字段表、Run 校验和固定 seed 测试保护，尚无单独的逐行 JSON Schema。
