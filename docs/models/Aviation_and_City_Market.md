...
- `regional_asset_market_impulse_index`、`regional_household_wealth_consumption_impulse` 与 `regional_real_disposable_income_growth_pct` 三条独立信号，以及股票价格回报、估值和风险偏好审计字段；
...
各客群从 100 点起步。区域需求 v0.4 把资产市场、居民财富、现金收入、信用信心和票价成本拆成五组可重建贡献，再计算总量和五类客群的相对调整；相对调整按区域基期权重中心化，因此纯结构变化在当期不改变加权总量。票价弹性真实进入对应客群，最终目标增长只用 `demand_adjustment_speed` 平滑一次并累积为需求指数。

高端倾向只接收一次资产、财富和现金贡献；免税与奢侈品通过高端倾向读取该信号，不再重复叠加完整财富效应。电子、餐饮和一般零售使用各自显式现金、客流与票价入口。所有商业倾向均发布 raw、贡献、final 和边界状态；这些指数不是销售额，也不包含机场合同或租金条款。
...
- 客流 v0.4 已把年度通胀压力真实传入五类需求并累积到后续客流路径，但尚未区分累计 CPI、实际可支配收入与机票相对价格；未来细化必须保留现有总量、分项和供需契约，并避免与收入、能源、汇率和宏观压力重复计价，见[通胀系统备忘录](../plans/Inflation_System_Memo.md)。
